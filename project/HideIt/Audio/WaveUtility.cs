...
namespace HideIt.Audio
{
	...
	public class WaveUtility
	{
		...
		/// </param>
		internal WaveUtility(Wave sourceStream, Stream destinationStream)
		:this(sourceStream){
		...
		internal WaveUtility(Wave sourceStream) {
			this.sourceStream = sourceStream;
			this.bytesPerSample = sourceStream.Format.wBitsPerSample / 8;
		}
        ///Hiding the Message
        ///Hiding a message in Wave samples is very similar to hiding it in the pixels of a bitmap. 
        ///Again, we use a key stream to skip a number of carrier units (samples/pixels), grab one 
        ///carrier unit, put one bit of the message into the lowest bit of the carrier unit, 
        ///and write the changed unit to the destination stream. When the entire message has been hidden 
        ///like that, we copy the rest of the carrier stream.
		/// <summary>
        ...
        ///Extracting the Message:-Again, we use the key stream to locate the right samples, 
        ///just as we did while hiding the message. Then we read the last bit of the sample and 
        ///shift it into the current byte of the message. When the byte is complete, we write it 
        ///into the message stream and continue with the next one.
